...
<div class="grid md:grid-cols-3 gap-8 items-start">
    <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        <h3 class="font-bold text-slate-900 text-base">📦 Tambah Produk Baru</h3>
        <form action="{{ route('admin.product.store') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
            @csrf
            <input type="text" name="name" placeholder="Nama Produk" class="w-full bg-slate-50 border-slate-200 rounded-xl text-sm p-2.5" required>
            ...
            <div>
                <label class="block text-xs text-slate-400 mb-1 font-semibold">File Gambar Produk (Max 2MB)</label>
            ...
            <button type="submit" class="w-full bg-teal-600 hover:bg-teal-700 text-white font-medium py-2.5 rounded-xl text-sm transition">Upload Produk</button>
        </form>
    </div>

    <div class="md:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm p-6 overflow-hidden">
        <h3 class="font-bold text-slate-900 mb-4">Daftar Produk Klinik</h3>

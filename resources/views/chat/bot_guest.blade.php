...
<body class="bg-slate-100 text-slate-800 antialiased h-screen flex flex-col justify-between">
    ...
    <header class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-6 shrink-0 shadow-sm">
        <div class="flex items-center space-x-3">
            ...
            <div>
                ...
                <p class="text-[11px] text-emerald-500 font-semibold"><span class="inline-block w-1.5 h-1.5 bg-emerald-500 rounded-full mr-1"></span>Online 24 Jam &middot; Tanpa Login</p>
            </div>
        </div>

        <div class="flex items-center space-x-3">
            <a href="{{ route('login') }}" class="text-xs bg-teal-600 hover:bg-teal-700 font-bold text-white px-4 py-2 rounded-xl transition">
                <i class="fa-solid fa-user-doctor"></i> Konsultasi Dokter
            </a>
            <a href="{{ route('chat.options') }}" class="text-xs bg-slate-100 hover:bg-slate-200 font-medium text-slate-600 px-4 py-2 rounded-xl transition">
    ...
    <div id="chat-box" class="flex-1 p-6 overflow-y-auto space-y-4 bg-slate-50">
        <div class="flex justify-start">
            <div class="max-w-[75%] rounded-2xl px-4 py-3 shadow-sm text-sm bg-white text-slate-800 rounded-bl-none border border-slate-100">
                ...
                <p class="text-[9px] opacity-40 mt-2">Catatan: Saya bukan pengganti diagnosis dokter. Untuk keluhan serius, silakan <a href="{{ route('login') }}" class="underline font-semibold">konsultasi langsung dengan dokter kami</a>.</p>
            </div>
        </div>
    ...
    <footer class="bg-white border-t border-slate-200 p-4 shrink-0 shadow-lg">
        ...
        <p class="text-center text-[10px] text-slate-300 mt-2">Riwayat chat ini akan hilang jika halaman di-refresh.</p>
    </footer>
    ...
    <script>
        ...
        // History percakapan disimpan HANYA di memori JS (hilang saat refresh)
        ...
        chatForm.addEventListener('submit', function (e) {
            ...
            .then(data => {
                ...
                const reply = data.message ?? 'Maaf, terjadi kesalahan. Silakan coba lagi.';
                appendBotMessage(reply);

                // Update history di memori (max 10 pasang terakhir biar tidak kebesaran)
